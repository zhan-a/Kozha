...
(function () {
  ...
  function renderReviewers(reviewers) {
    ...
    reviewers.forEach(function (r) {
      ...
      name.textContent = r.display_name || '(unnamed reviewer)';
      if (r.is_deaf_native) {
        var tag = document.createElement('span');
        tag.className = 'governance-reviewer-native';
        tag.textContent = 'Deaf native signer';
      ...
      if (r.signs && r.signs.length) {
        parts.push('reviews: ' + r.signs.map(function (s) { return s.toUpperCase(); }).join(', '));
      }
      if (r.regional_background) parts.push('region: ' + r.regional_background);
  ...
  function renderBoard(board) {
    ...
    intro.textContent = 'The following Deaf advisors have consented to be listed publicly as members of the board:';
    ...
    board.forEach(function (m) {
      ...
      name.textContent = m.name || '(unnamed advisor)';
      li.appendChild(name);
      var meta = document.createElement('p');
  ...
  function renderLanguages(languages) {
    ...
    languages.forEach(function (lang) {
      ...
      if (total === 0) {
        status.textContent = 'no reviewers seated';
      } else if (native === 0) {
        status.textContent = total + ' reviewer' + (total === 1 ? '' : 's') +
          ', none Deaf native — review paused';
      } else {
        status.textContent = native + ' Deaf native · ' + total + ' total';
      }
      li.appendChild(name);
