...
- **ASL** — [ASL-LEX 2.0](https://asl-lex.org) (Sehyr, Caselli, Cohen-Goldberg & Emmorey 2021). License: **CC BY-NC 4.0** (per asl-lex.org; the ASL-LEX OSF node is tagged CC BY 4.0 — we adopt the stricter non-commercial terms). **14 verified pilot signs**, hand-authored and cross-checked against native-signer video (and confirmed to render), plus the 25-letter ASL fingerspelling alphabet. Everything not individually verified — 21 further hand-authored candidates and a 1,657-sign set machine-converted from the ASL-LEX 2.0 phonology — is held in [`data/asl_pending_review/`](data/asl_pending_review/), **not served or counted** until each is checked against native-signer video. The raw ASL-LEX corpus is not redistributed — only derived SiGML ships.
