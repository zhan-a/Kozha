...
- **ASL** — [ASL-LEX 2.0](https://asl-lex.org) (Sehyr, Caselli, Cohen-Goldberg & Emmorey 2021). License: **CC BY-NC 4.0** (per asl-lex.org; the ASL-LEX OSF node is tagged CC BY 4.0 — we adopt the stricter non-commercial terms). 1,657 lexical signs machine-converted from the ASL-LEX 2.0 phonological coding scheme, plus the 25-letter ASL fingerspelling alphabet. This is a **seed**: handshape/location/symmetry/movement-shape are derived deterministically from ASL-LEX columns, but palm/finger orientation and movement direction are rule-derived heuristics (unverified), and the set is **awaiting Deaf-native review** (0 reviewers). The raw ASL-LEX corpus is not redistributed — only the derived SiGML ships.
