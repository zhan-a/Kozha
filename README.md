...
- **ASL** — [ASL-LEX 2.0](https://asl-lex.org) (Sehyr, Caselli, Cohen-Goldberg & Emmorey 2021). License: **CC BY-NC 4.0** (per asl-lex.org; the ASL-LEX OSF node is tagged CC BY 4.0 — we adopt the stricter non-commercial terms). **35 pilot signs**, hand-authored and cross-checked against native-signer video, plus the 25-letter ASL fingerspelling alphabet. A larger 1,657-sign set machine-converted from the ASL-LEX 2.0 phonology is held in [`data/asl_pending_review/`](data/asl_pending_review/) — its orientation/movement is rule-derived and **unverified**, so it is **not served or counted** until Deaf-native review. The raw ASL-LEX corpus is not redistributed — only derived SiGML ships.
